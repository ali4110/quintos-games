function preload() {
	...
	for (let speak of speech) {
		speechSounds[speak] = loadSound('sounds/speech/' + speak + '.mp3');
...
function onSubmit(value) {
	...
	if (word == value) {
		let correctAudio = speechSounds['that_is_correct_now_spell'];
		correctAudio.play();
		correctAudio.onended(() => {
			nextWord();
		});
	} else {
		let incorrectAudio = speechSounds['that_is_incorrect_the_correct_spelling_of'];
		incorrectAudio.play();
...
function spellWord(letterIndex) {
	let letter = word[letterIndex].toUpperCase();
	letterSounds[letter].play();
	letterSounds[letter].onended(() => {
		if (spellingWord == false) return;
...
function nextWord() {
	...
	word = words[round(random(0, words.length))];
